...
def FowardKinematics(state, state_mode="tb"):
    ...
    OAR_ref = np.array([[l1*np.sin(theta)],
                        [l1*np.cos(theta)]])
    
    OBR_ref = np.array([[R*np.sin(theta)],
                        [R*np.cos(theta)]])
    ...
    OER_ref = np.array([[0],
                        [l1*np.cos(theta)- (l5+l6)*np.cos(alpha_2)]])
    ...
    rot_alpha_3 = np.array([[cos_alpha_3, sin_alpha_3],
    ...
    rot_alpha_4 = np.array([[np.cos(alpha_4), -np.sin(alpha_4)],
                            [np.sin(alpha_4), np.cos(alpha_4)]])
    ...
    OG_ref = np.array([[0], 
                       [OFR_ref[1,0] - l8*cos_alpha_5]])
    
    # Derive Left Plane Vectors
    mirror_mat = np.array([[-1, 0],
                           [0, 1]])
